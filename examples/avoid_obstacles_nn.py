...
# print(np.shape(obst_vec[:,0]))
# print(np.shape(obst_vec[:,1]))
# print(np.shape(obst_dist))
# print(np.shape(obst_angle))
# print(np.shape(cur_vel))
inpt = np.vstack((obst_vec[:,0], obst_vec[:,1]))#, obst_dist[:,0], obst_dist[:,1], obst_angle))
inpt = np.swapaxes(inpt, 0, 1)
#np.random.shuffle(inpt)
...
def nn_avoid(y, dy, goal, model, postprocess=True):
	...
	for obstacle in obstacles:
		if np.linalg.norm(dy) > 1e-5:
		        ...
		        tst = np.array([[obstv[0]], [obstv[1]]])#, [obstd[0]], [obstd[1]], [obsta], [cv]])
